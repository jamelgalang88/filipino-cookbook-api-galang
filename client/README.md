...
The client allows users to view Filipino food records, search foods by name, find one food by ID, view food categories, view ingredients, submit a new food record through a POST request, and test API authentication using an invalid token request.
...
- Submits new food records using the `POST /api/foods` endpoint
